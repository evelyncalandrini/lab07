...
import it.unibo.inner.api.IterableArray;
...
public class TestIterableWithPolicy {
    ...
    private static <T> IterableWithPolicy<T> makeIterableWithPolicy(final T[] elements, final Predicate<T> filter) {
        return new IterableArray<>(elements, filter); // TODO: return the implementation of IterableWithPolicy
    }

    private static <T> IterableWithPolicy<T> makeIterableWithPolicy(final T[] elements) {
        return new IterableArray<>(elements); // TODO: return the implementation of IterableWithPolicy
    }
